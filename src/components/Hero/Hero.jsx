import React from "react";
import { useTranslation } from "react-i18next";
import LanguageSelector from "../LanguageSelector/LanguageSelector";
import "./Hero.css";
import heroIcon from "../../assets/hero-icon.png";
import profilePicture from "../../assets/profile.jpg";

function Hero() {
  const [t, i18n] = useTranslation("global");

  return (
    <div id="home" className="wrapper hero">
      <div className="hero hero-container">
        <div className="hero-content">
          <div className="hero-text">
            <div className="hero-title-container">
              <h1 className="hero-title">{t("hero.title1")}</h1>
              <img className="hero-icon" src={heroIcon} />
            </div>
            <div className="hero-description">
              <div className="description-text">{t("hero.description")} 📍</div>
            </div>
            <div className="hero-socials">
              <a
                className="social-link"
                href="https://www.linkedin.com/in/kiss-krisztian/"
                target="_blank"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="35"
                  height="35"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  className="tabler-icon tabler-icon-brand-linkedin"
                >
                  <path d="M4 4m0 2a2 2 0 0 1 2 -2h12a2 2 0 0 1 2 2v12a2 2 0 0 1 -2 2h-12a2 2 0 0 1 -2 -2z"></path>
                  <path d="M8 11l0 5"></path>
                  <path d="M8 8l0 .01"></path>
                  <path d="M12 16l0 -5"></path>
                  <path d="M16 16v-3a2 2 0 0 0 -4 0"></path>
                </svg>
              </a>
              <a
                className="social-link"
                href="https://github.com/KissKrisz215"
                target="_blank"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="35"
                  height="35"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  stroke-width="2"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  class="tabler-icon tabler-icon-brand-github"
                >
                  <path d="M9 19c-4.3 1.4 -4.3 -2.5 -6 -3m12 5v-3.5c0 -1 .1 -1.4 -.5 -2c2.8 -.3 5.5 -1.4 5.5 -6a4.6 4.6 0 0 0 -1.3 -3.2a4.2 4.2 0 0 0 -.1 -3.2s-1.1 -.3 -3.5 1.3a12.3 12.3 0 0 0 -6.2 0c-2.4 -1.6 -3.5 -1.3 -3.5 -1.3a4.2 4.2 0 0 0 -.1 3.2a4.6 4.6 0 0 0 -1.3 3.2c0 4.6 2.7 5.7 5.5 6c-.6 .6 -.6 1.2 -.5 2v3.5"></path>
                </svg>
              </a>
            </div>
          </div>
          <div>
            <img className="profile-picture" src={profilePicture} alt="" />
          </div>
        </div>
        <div className="technologies-container">
          <p className="technologies-title">{t("hero.footer")}</p>
          <div className="technologies-cards">
            <div className="card">
              <div className="card-title-hover">
                <span className="text">HTML</span>
                <span className="text">+</span>
                <span className="text">CSS</span>
              </div>
              <img
                className="card-image"
                src="https://skillicons.dev/icons?i=html,css"
                alt="Html Css skill-icon"
              />
            </div>
            <div className="card">
              <div className="card-title-hover">
                <span className="text">JS</span>
                <span className="text">+</span>
                <span className="text">React</span>
              </div>
              <img
                className="card-image"
                src="https://skillicons.dev/icons?i=js,react"
                alt="Javascript React skill-icon"
              />
            </div>
            <div className="card">
              <div className="card-title-hover">
                <span className="text">Tailwind</span>
                <span className="text">+</span>
                <span className="text">Node.js</span>
              </div>
              <img
                className="card-image"
                src="https://skillicons.dev/icons?i=tailwind,nodejs"
                alt="Tailwind Node.js skill-icon"
              />
            </div>
            <div className="card">
              <div className="card-title-hover">
                <span className="text">Express</span>
                <span className="text">+</span>
                <span className="text">MongoDB</span>
              </div>
              <img
                className="card-image"
                src="https://skillicons.dev/icons?i=express,mongodb"
                alt="Express Mongodb skill-icon"
              />
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

export default Hero;
